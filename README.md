...
> 融入 Claude Code 的核心逻辑：**流式输出**（边生成边显示）、**LLM 摘要式上下文压缩**（长对话不丢语义）、**TodoWrite 任务追踪**、**富系统提示**（注入平台/git/工具用法 + 持久化记忆）、**斜杠命令**、**工作区**（记忆 + 临时脚本落盘）。单文件、零依赖、Python 3.6+ 通用。
...
```
    ...
    │                        Agent Loop (外层)                            │
    │  ┌──────────────────────────────────────────────────────────────┐  │
    │  │              Context Compression (超过阈值触发)                │  │
    │  │     旧消息 ──▶ LLM 摘要 ──▶ 替换为 [历史摘要] + 最近消息       │  │
    │  └──────────────────────────────────────────────────────────────┘  │
    │                                                                    │
    │  ┌──────────────────────────────────────────────────────────────┐  │
    │  │                 Tool-Calling Loop (内层，max 10 轮)            │  │
    │  │                                                               │  │
    │  │   ┌──────────┐      tool_calls?                               │  │
    │  │   │          │─── Yes ──────────────────────────────┐         │  │
    │  │   │   LLM    │                                      │         │  │
    │  │   │   推理   │─── No ──▶ 生成回复 ──▶ strip_md ──▶ 终端     │  │
    │  │   │          │                                      │         │  │
    │  │   └──────────┘                                      │         │  │
    │  │        ▲                                            ▼         │  │
    │  │        │                               ┌─────────────────────┐│  │
...
| 循环 | 位置 | 作用 |
|------|------|------|
| **外层** Agent Loop | `while True` (line 1071) | 用户多轮对话，每轮一个完整排查任务 |
| **内层** Tool-Calling Loop | `for round in 1..10` (line 1094) | 单轮内 LLM ↔ 工具多轮交互，直到 LLM 不再调用工具 |
| **压缩** Context Compression | 内层入口 (line 1100) | 消息超阈值时 LLM 摘要压缩，不丢上下文 |
